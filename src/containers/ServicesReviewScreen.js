...
class ServicesReviewsScreen extends PureComponent {
    ...
    onRatingSelect = (rating)=>{
        ...
        const data = {
            rate: rating,
            shop_type: 'serviceshop'
        };
        NavigationService.navigate('GiveReviewScreen', {rating:data , callBack:this.serviceShopDetail})
    }
    ...
    placeholderView = ()=>{
        return (
            <View style={[styleServicesReviewsScreen.container, {
                alignItems: "center",
                paddingTop: 8
            }]}>

                <Image style={styleServicesReviewsScreen.reviewPicture}
                       source={require("../resources/icons/rating-empty.png")}
                       resizeMode="contain" />

                <Text style={styleServicesReviewsScreen.rateTxtHeading} >{__('Be the first to rate this service' , this.props.language)}</Text>

                <Text style={styleServicesReviewsScreen.noReviewText}>{__('it takes few seconds to share and support others with your experience' , this.props.language)}</Text>

                <TouchableOpacity onPress={this.onRateNowPress}>
                    <View style={styleServicesReviewsScreen.btnStyle}>
                        <Text style={styles.tapButtonStyleTextWhite}>{__('Rate now' , this.props.language)}</Text>
                    </View>
                </TouchableOpacity>
                {this.renderAds()}
                {/* <Image
                    style={styleServicesReviewsScreen.ad}
...
export default connect(mapStateToProps, null)(ServicesReviewsScreen)

const styleServicesReviewsScreen = StyleSheet.create({
    container: {
        flex: 1,
